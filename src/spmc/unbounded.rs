...
use crate::spmc::SPMCBoundedQueue;
use crate::spsc::SPSCUnboundedQueue;
...
impl<T, SC, AtomicU32Wrapper, AtomicU64Wrapper>
...
unsafe impl<T, SC, AtomicU32Wrapper, AtomicU64Wrapper> Send
...
unsafe impl<T, SC, AtomicU32Wrapper, AtomicU64Wrapper> Sync
    for SPMCUnboundedQueue<T, SC, AtomicU32Wrapper, AtomicU64Wrapper>
where
    SC: SyncCell<LightArc<Version<T>>>,
    AtomicU32Wrapper: Deref<Target = AtomicU32> + Default,
...
macro_rules! generate_spmc_producer_and_consumer {
    ($producer_name:ident, $consumer_name:ident, $atomic_u32_wrapper:ty, $long_atomic_wrapper:ty, $sync_cell:ty) => {
        ...
        impl<T, SC> $crate::spmc_producer::SPMCProducer for $producer_name<T, SC>
        where
            SC: SyncCell<LightArc<Version<T>>> + LockFreeSyncCell<LightArc<Version<T>>>
        {
            #[inline]
        ...
        impl<T, SC> $crate::spmc_producer::SPMCLockFreeProducer for $producer_name<T, SC>
        where
            SC: SyncCell<LightArc<Version<T>>> + LockFreeSyncCell<LightArc<Version<T>>>
        {
            ...
            fn lock_free_push<BR: LockFreeBatchReceiver<T>>(&self, value: T, _batch_receiver: &BR) -> Result<(), T> {
                unsafe { self.inner.producer_push(value, batch_receiver) };
...
///
/// ```
/// use parcoll::spmc::new_unbounded;
...
/// ```
pub fn new_unbounded<T, SC: SyncCell<LightArc<Version<T>>>>() -> (SPMCUnboundedProducer<T>, SPMCUnboundedConsumer<T>) {
...
///
/// ```
/// use parcoll::spmc::new_cache_padded_unbounded;
/// use parcoll::{Producer, Consumer};
///
